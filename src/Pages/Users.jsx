...
const Users = () => {
    ...
    const handleStatus = (ID, Status) => {
        Swal.fire({
            title: "Are you sure?",
            text: "You want to change the user's Status",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                ...
                fetch(`http://localhost:5000/status/users?id=${ID}`, {
                    method: "PUT",
                    headers: {
                        'content-type': 'application/json'
                    },
                    ...
                    .then(data => {
                        if (data.modifiedCount > 0) {
                            ...
                            Swal.fire({
                                title: `The User ${UserStatus}`,
                                text: `Successfully the user ${UserStatus}`,
                                icon: "success"
                            });
                            AllUsers();
                        }
                            ...
                            <div>
                                {
                                    qualifiedUsers.map(user => {
                                        return <ul key={user._id} className="grid grid-cols-6 place-items-center text-[12px] py-4 border inter">
                                            ...
                                            <li>{user?.name}</li>
                                            <li>{user?.email}</li>
                                            <button onClick={() => handleStatus(user._id, user.status)} className="bg-gradient-to-l from-[#8D53FD] to-[#9E6EFD]  py-2 md:py-3 px-3 md:px-6 lg:px-9 text-white font-bold text-xs  rounded">{user?.status === 'active' ? <>BLOCK USER</> : <>ACTIVE USER</>}</button>
                                            {
                                                user.role === 'user' ? <button onClick={() => handleMakeAdmin(user._id)} className="border-[#8D53FD] border-2  py-2 md:py-3 px-3 md:px-6 lg:px-9 text-[#8D53FD] font-bold text-xs rounded">MAKE ADMIN</button> :
                                                    <li>ADMIN</li>
                                            }
                                            <button onClick={() => {
                                                document.getElementById('my_modal_5').showModal()
                                                setModalData(user)
                                            }} className="bg-gradient-to-l from-[#8D53FD] to-[#9E6EFD]  py-2 md:py-3 px-3 md:px-6 lg:px-9 text-white font-bold text-xs   rounded">SEE INFO</button>
                                        </ul>
                                    })
                                }
                            </div></>
                            : <>
                                <div className="flex justify-center text-purple-600 items-center min-h-full">
                                    <Circles
                                        height="80"
                                        width="80"
                                        color="#8D53FD"
